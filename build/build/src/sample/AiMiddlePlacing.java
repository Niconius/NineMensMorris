				...
				if(preventOpponentCross(1)==false) {
					if(recognizeCrossOpportunity(1)==false) {
						if(preventOpponentOpening()==false) {
									...
									if(recognizeCrossOpportunity(0)==false) {
										if(doubleCrossPrep()==false) {
											if(twoPiecesNearbyAndThirdIsFree()==false) {
												...
												System.out.println("MostMobility");
											}
											else {
												System.out.println("PrepareMill");
											}
										}
										else {
											System.out.println("DoubleCrossPrep");
										}
									}
									else {
										System.out.println("RecognizedCrossEvenOnLastPlace");
									}
								}
								else {
									System.out.println("MillBlockable");
								}
							}
							else {
							...
							System.out.println("PreventOpening");
						}
					}
					else {
						System.out.println("RecognizedCrossAndAtleatOneMorePlace");
					}
				}
				else {
					System.out.println("PreventCross");
				}
			}
			else {
				System.out.println("PreventMill");
			}
		}
		else {
			System.out.println("MillOpen");
		}
		return wherePlace;
	}
	...
	private boolean opponentMillBlockable() {
		...
		for(int i=1;i<millPos.length;i++) {
			for(int j=0;j<9;j++) {
				...
				Piece piece2=null;
				Piece piece3=null;
				if(playerPieces[j].getIsInMill()==true) {
