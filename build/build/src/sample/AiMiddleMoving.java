							...
							if(blockMillPrep()==false) {
								if(openMillIfOpponentCantKillIt()==false) {
									if(millPrep()==false) {
										...
										System.out.println("MillPrep");
									}
								}
								else {
									System.out.println("OpenMillBecauseOpponentCantKillIt");
								}
							}
							else {
								System.out.println("BlockMillPrep");
							}
						}
						else {
							System.out.println("BlockedOnlyMillOfOpponent");
						}
					}
					else {
						System.out.println("CloseMillBecauseOpponentHasNoClosedMill");
					}
				}
				else {
					System.out.println("CloseMillBecauseOpponentCouldBlockIt");
				}
			}
			else {
				System.out.println("CloseMillBecauseOpponentCouldCloseHis");
			}
		}
		else {
		...
		if(winThroughOpponentLessThan3()==false) {
			if(completelyBlockedThroughMove()==false) {
				if(completelyBlockedThroughRemove()==false) {
					...
					System.out.println("BlockedThroughRemove");
					return true;
				}
			}
			else {
				System.out.println("BlockedThroughMove");
				return true;
			}
		}
		else {
			System.out.println("OpponentLessThan3");
			return true;
		}
	}
							...
							if(board.canAPieceMoveToPositionWithout2(l, player, player.getPieces()[j], player.getPieces()[k])[0]!=null) {
								for(int y=0;y<2;y++) {
									...
									if(check1==true && check2==true) {
										...
										counterOpenMills++;
										System.out.println("here3"+ counterOpenMills + l);
										break;
									}
								}
							}
			...
			for(int i=0;i<9;i++) {
				for(int j=0;j<4;j++) {
					if(board.isMovePossible(j, ai.getPieces()[i])==true) {
						...
						int newPosition = board.getBoardAsArray()[ai.getPieces()[i].getPosition()][j];
						ai.getPieces()[i].setPosition(newPosition);
						if(player.checkForMill(ai.getPieces()[i])==true) {
							if(ai.getPieces()[i].getIsInMill()==true) {
								canThisPieceMakeAMill=true;
								ai.getPieces()[i].setPosition(savePos);
								player.checkForMill(ai.getPieces()[i]);
								break;
							}
						}
						ai.getPieces()[i].setPosition(savePos);
